...
export const createProduct = (product: Omit<Product, "id">) =>
  request<Product>("/products", {
    method: "POST",
    body: JSON.stringify({ id: 0, ...product }),
  });
...
export const addModule = (
  productId: number,
  module: Omit<Module, "id" | "productId">,
) =>
  request<Module>(`/products/${productId}/modules`, {
    method: "POST",
    body: JSON.stringify({ id: 0, productId, ...module }),
  });

export const deleteModule = (moduleId: number) =>
  request<void>(`/products/modules/${moduleId}`, { method: "DELETE" });
...
export const addProductResponsibility = (
  productId: number,
  teamMemberId: number,
...
export const deleteProductResponsibility = (
  productId: number,
  responsibilityId: number,
) =>
  request<void>(`/products/${productId}/responsibilities/${responsibilityId}`, {
    method: "DELETE",
  });

export const addRepository = (
  productId: number,
  repo: Omit<Repository, "id" | "productId">,
) =>
  request<Repository>(`/products/${productId}/repositories`, {
    method: "POST",
    body: JSON.stringify({ id: 0, productId, ...repo }),
  });

export const deleteRepository = (productId: number, repositoryId: number) =>
  request<void>(`/products/${productId}/repositories/${repositoryId}`, {
    method: "DELETE",
  });

export const addDocument = (
  productId: number,
  document: Omit<ProductDocument, "id" | "productId">,
) =>
  request<ProductDocument>(`/products/${productId}/documents`, {
    method: "POST",
    body: JSON.stringify({ id: 0, productId, ...document }),
  });

export const deleteDocument = (productId: number, documentId: number) =>
  request<void>(`/products/${productId}/documents/${documentId}`, {
    method: "DELETE",
  });

// clients
export const getClients = (
  filters: { companyName?: string; country?: string; productId?: number } = {},
) => request<Client[]>(`/clients${query(filters)}`);

export const getClientDetails = (id: number) =>
  request<ClientDetails>(`/clients/${id}`);

export const createClient = (client: Omit<Client, "id">) =>
  request<Client>("/clients", {
    method: "POST",
    body: JSON.stringify({ id: 0, ...client }),
  });
...
export const addDeployment = (
  clientId: number,
  deployment: Omit<Deployment, "id" | "clientId">,
) =>
  request<Deployment>(`/clients/${clientId}/deployments`, {
    method: "POST",
    body: JSON.stringify({ id: 0, clientId, ...deployment }),
  });
...
export const enableModule = (
  ...
  request<void>(
    `/clients/${clientId}/deployments/${deploymentId}/modules/${moduleId}`,
    { method: "POST" },
  );
...
// Environments live under one deployment. Add needs the deploymentId to file
// the new row under (comes from the route + body), update/delete only need
// the environment's own id, deploymentId never changes on an existing row.
export const addEnvironment = (
  ...
  request<Environment>(
    `/clients/${clientId}/deployments/${deploymentId}/environments`,
    {
      method: "POST",
      body: JSON.stringify({ id: 0, deploymentId, ...environment }),
    },
  );
...
export const updateEnvironment = (
  ...
  request<Environment>(`/clients/${clientId}/environments/${environmentId}`, {
    method: "PUT",
    body: JSON.stringify({ id: environmentId, deploymentId, ...environment }),
  });
...
export const getDeployments = (
  filters: { productId?: number; clientId?: number; status?: string } = {},
) => request<DeploymentListItem[]>(`/deployments${query(filters)}`);

// team members
export const getTeamMembers = (
  filters: { fullName?: string; department?: string } = {},
...
export const createUser = (
  ...
  request<User>("/users", {
    method: "POST",
    body: JSON.stringify({ email, password, role, teamMemberId }),
  });

export const updateUser = (
  id: number,
  changes: { role?: string; isActive?: boolean; teamMemberId?: number },
) =>
